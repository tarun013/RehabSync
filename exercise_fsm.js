...
export class ExerciseFSM {
    ...
    updateSquat(landmarks) {
        ...
        switch (this.state) {
            ...
            case 'descending':
                ...
                else if (angle > standThres) this.state = 'neutral';
                break;
            case 'bottom':
                // Track lowest point for adaptation? 
                // Better: Update 'depth' observation continuously while in 'bottom' or 'descending' if it goes lower?
                // For simplicity, we just trigger when entering bottom or if we go deeper.
    ...
    updateCurl(landmarks) {
        ...
        switch (this.state) {
            ...
            case 'flexing': // Moving up
                ...
                else if (angle > thres.extend) this.state = 'neutral'; // Aborted rep, went back down
                break;
    ...
    updatePress(landmarks) {
        // Shoulder Press: Elbow angle is key, but also elevation of wrist relative to shoulder
        ...
        const angle = calculateAngle2D(shoulder, elbow, wrist); // Elbow extension
        const thres = this.getThresholds('shoulder_press');

        // --- STRICT POSITION CHECKS ---
        // 1. Hands must be ABOVE shoulders for 'neutral' start of press? 
        // Actually, start is hands at shoulder height. Wrist Y ~= Shoulder Y.
        // Peak: Wrist Y < Nose Y (High up).

        // Check relative height (Y is smaller as you go up)
        const wristAboveShoulder = wrist.y < shoulder.y;
        const wristAboveNose = nose ? wrist.y < nose.y : wrist.y < (shoulder.y - 0.2); // Fallback

        switch (this.state) {
            case 'neutral': // Hands at shoulders (elbow bent ~60-90)
                // To start pressing, we must be in position first.
                // Angle should be acute (< 90). Wrist near shoulder.
                if (angle < 100 && Math.abs(wrist.y - shoulder.y) < 0.3) {
                    // Ready to press
                }

                // Transition to pressing
                if (angle > thres.start + 10 && wristAboveShoulder) {
                    this.state = 'pressing';
                    this.isValid = true;
                }
                break;
            case 'pressing':
                if (angle > thres.overhead && wristAboveNose) this.state = 'peak';
                else if (angle < thres.start) this.state = 'neutral';
                break;
            case 'peak': // Arms extended overhead
                if (angle < thres.overhead - 10) this.state = 'lowering';
                break;
            case 'lowering':
                if (angle < thres.start) {
                    // Must end near shoulders
                    if (Math.abs(wrist.y - shoulder.y) < 0.4) {
                        this.state = 'neutral';
                        if (this.isValid) this.reps++;
                    }
                } else if (angle > thres.overhead) this.state = 'peak';
                break;
        }
        return { state: this.state, reps: this.reps, currentAngle: angle, isValid: this.isValid };
    }
